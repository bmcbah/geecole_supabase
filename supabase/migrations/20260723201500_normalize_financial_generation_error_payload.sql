  ...
  loop
    ...
    exception when others then
      ...
      error_list := error_list || jsonb_build_array(jsonb_build_object(
        ...
        'student_name', nullif(trim(enrollment_record.student_name), ''),
        'matricule', enrollment_record.matricule,
        'level_name', enrollment_record.level_name_snapshot,
        'cycle_name', enrollment_record.cycle_name_snapshot,
        'code', error_sqlstate,
...
comment on function public.reapply_all_student_financial_accounts(uuid, uuid)
is 'Génère ou réapplique les dossiers financiers et retourne un rapport JSON normalisé en snake_case pour le client.';
