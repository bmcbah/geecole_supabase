...
export function InscriptionsWorkspacePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(scoped);
      setSelected((current) => current ? scoped.find((item) => item.id === current.id) ?? null : scoped.find((item) => item.status === "draft") ?? scoped[0] ?? null);
    } catch {
  ...
  const counters = useMemo(() => ({
    ...
    history: items.filter((item) => ["cancelled", "transferred", "rejected", "withdrawn"].includes(item.status)).length,
  }), [items]);

  const filtered = useMemo(() => {
    const normalized = query.trim().toLocaleLowerCase("fr");
    return items.filter((item) => {
      const matchesView = view === "to-review" ? item.status === "draft" : view === "confirmed" ? item.status === "confirmed" : ["cancelled", "transferred", "rejected", "withdrawn"].includes(item.status);
  ...
  const apply = async () => {
    ...
    try {
      ...
      setFailure(error instanceof Error ? error.message : "La transition n’a pas pu être enregistrée.");
    }
  };
      ...
      <div className="space-y-4">
        ...
        <div className="grid min-h-[560px] gap-4 lg:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div className="border-b border-slate-200 px-4 py-3"><h2 className="text-sm font-semibold text-slate-950">File de dossiers</h2><p className="text-xs text-slate-500">{loading ? "Chargement…" : `${filtered.length} dossier(s)`}</p></div>
            <div className="max-h-[500px] overflow-y-auto p-2">
              {filtered.map((item) => <button key={item.id} type="button" className={`mb-1 w-full rounded-lg border p-3 text-left ${selected?.id === item.id ? "border-emerald-300 bg-emerald-50" : "border-transparent hover:bg-slate-50"}`} onClick={() => setSelected(item)}><div className="flex items-start justify-between gap-2"><span><strong className="block text-sm text-slate-900">{item.student.first_name} {item.student.last_name}</strong><small className="text-slate-500">{item.student.matricule} · {item.level_name_snapshot}</small></span><Tag value={labels[item.status] ?? item.status} severity={severity(item.status)} /></div></button>)}
              {!filtered.length ? <div className="p-6 text-center text-sm text-slate-500">Aucun dossier dans cette file.</div> : null}
            </div>
          </aside>

          <section className="rounded-xl border border-slate-200 bg-white p-5">
            {selected ? <div className="space-y-5">
              <div className="flex flex-wrap items-start justify-between gap-3"><div><span className="text-xs font-semibold uppercase text-slate-500">Dossier sélectionné</span><h2 className="mt-1 text-xl font-semibold text-slate-950">{selected.student.first_name} {selected.student.last_name}</h2><p className="text-sm text-slate-500">{selected.student.matricule}</p></div><Tag value={labels[selected.status] ?? selected.status} severity={severity(selected.status)} /></div>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Cycle", selected.cycle_name_snapshot], ["Niveau", selected.level_name_snapshot], ["Classe", selected.assignment[0]?.class_name ?? "Non affectée"], ["Date", new Date(selected.admission_date).toLocaleDateString("fr-FR")]].map(([label, value]) => <div key={label} className="rounded-lg bg-slate-50 p-3"><span className="text-xs text-slate-500">{label}</span><strong className="block text-sm text-slate-900">{value}</strong></div>)}</div>
              {selected.status === "draft" ? <Message severity="info" text="Contrôlez l’identité, le responsable principal, le niveau et les pièces avant de valider l’inscription." /> : null}
              <div className="flex flex-wrap gap-2 border-t border-slate-200 pt-4"><Button label="Ouvrir la fiche élève" icon="pi pi-user" outlined onClick={() => void navigate(`/scolarite/eleves/${selected.student.id}`)} />{selected.status === "draft" ? <Button label="Valider l’inscription" icon="pi pi-check" onClick={() => openTransition("confirmed")} /> : null}{selected.status === "draft" ? <Button label="Refuser" icon="pi pi-times" severity="danger" text onClick={() => openTransition("rejected")} /> : null}{selected.status === "confirmed" ? <Button label="Annuler" icon="pi pi-ban" severity="danger" text onClick={() => openTransition("cancelled")} /> : null}</div>
            </div> : <div className="grid min-h-[420px] place-items-center text-center text-sm text-slate-500">Sélectionnez un dossier dans la file de gauche.</div>}
          </section>
        </div>
      </div>
