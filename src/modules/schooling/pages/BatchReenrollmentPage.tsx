...
export function BatchReenrollmentPage() {
  ...
  const sourceYears = useMemo(
    () => years.filter((item) => item.id !== yearId),
    [yearId, years],
  );
  useEffect(
    () => setSourceYearId((current) => current || sourceYears[0]?.id || ""),
    [sourceYears],
  );
    ...
    void Promise.all([
      ...
      setStudents(
        previousStudents.filter(
          (item) =>
            item.status === "confirmed" && !alreadyEnrolled.has(item.id),
        ),
      );
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter(
      (item) =>
        !normalized ||
        `${item.firstName} ${item.lastName} ${item.matricule} ${item.levelName}`
  ...
  return (
    <section className="medium-controls space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-xl border border-slate-200 bg-white p-3">
        <label className="field min-w-56">
          ...
          <Dropdown
            ...
            onChange={(event) => setSourceYearId(String(event.value))}
          />
        </label>
        ...
        <Button
          ...
          icon="pi pi-refresh"
          disabled={!allowed || !yearId || !selected.length}
          loading={loading}
          size="small"
          onClick={() => {
            setLoading(true);
      ...
      <Message
        severity="info"
        text={`Les dossiers seront créés dans l’année active avec le statut « ${resultStatus} ». L’inscription de l’année antérieure reste inchangée.`}
      />
      {results.length > 0 ? (
        <Message
          severity={created === results.length ? "success" : "warn"}
          text={`${created} dossier(s) créé(s), ${results.length - created} à corriger.`}
        />
      ) : null}
      <DataTable
        ...
        rows={15}
        size="small"
        emptyMessage="Tous les élèves de cette année sont déjà réinscrits ou aucun élève confirmé n’a été trouvé."
      >
