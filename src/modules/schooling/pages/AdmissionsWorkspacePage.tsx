...
const statusOptions: Array<{
  label: string;
  value: EnrollmentWorkflowStatus | "";
}> = [
...
export function AdmissionsWorkspacePage() {
  ...
  const openTransition = (
    row: EnrollmentWorkflowRow,
    next: EnrollmentWorkflowStatus,
  ) => {
    setSelected(row);
    setTarget(next);
    setReason("");
  ...
  const apply = async () => {
    ...
    try {
      ...
      setFailure(
        "La transition demandée n'est pas autorisée ou n'a pas pu être enregistrée.",
      );
    } finally {
      setProcessing(false);
    }
    ...
    >
      ...
      >
        ...
        <Column
          header="Classe"
          body={(row: EnrollmentWorkflowRow) =>
            row.assignment[0]?.class_name ?? "Non affectée"
          }
        />
        <Column
          header="Statut"
          body={(row: EnrollmentWorkflowRow) => (
            <Tag value={labels[row.status]} severity={severity[row.status]} />
          )}
        />
        <Column
          header="Actions"
            ...
            <div className="flex flex-wrap gap-2">
              ...
              row.status !== "transferred" ? (
                <Button
                  ...
                  icon="pi pi-check"
                  size="small"
                  onClick={() => openTransition(row, "confirmed")}
                />
              ) : null}
              {row.status === "draft" ? (
                <Button
                  ...
                  outlined
                  size="small"
                  onClick={() => openTransition(row, "pre_registered")}
                />
              ) : null}
              {row.status !== "confirmed" && row.status !== "rejected" ? (
                <Button
                  ...
                  text
                  size="small"
                  onClick={() => openTransition(row, "rejected")}
                />
              ) : null}
              {row.status === "confirmed" ? (
                <Button
                  ...
                  outlined
                  size="small"
                  onClick={() => openTransition(row, "transferred")}
                />
              ) : null}
              {row.status === "confirmed" ? (
                <Button
                  ...
                  text
                  size="small"
                  onClick={() => openTransition(row, "cancelled")}
                />
              ) : null}
            </div>
          )}
      ...
      <Dialog
        header={`Passer le dossier à « ${labels[target]} »`}
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              loading={processing}
              disabled={
                ["rejected", "withdrawn", "cancelled", "transferred"].includes(
                  target,
                ) && !reason.trim()
              }
              onClick={() => void apply()}
      ...
      >
        <p className="mb-4 text-sm text-slate-600">
          {selected
            ? `${selected.student.first_name} ${selected.student.last_name} · ${selected.student.matricule}`
            : ""}
        </p>
