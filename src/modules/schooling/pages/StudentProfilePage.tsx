      ...
      <div className="space-y-4">
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          <TabView
            ...
            className="[&_.p-tabview-nav]:border-slate-200 [&_.p-tabview-nav]:bg-white [&_.p-tabview-nav]:px-3 [&_.p-tabview-nav-link]:px-3 [&_.p-tabview-nav-link]:py-3 [&_.p-tabview-nav-link]:text-slate-600 [&_.p-highlight_.p-tabview-nav-link]:border-emerald-400 [&_.p-highlight_.p-tabview-nav-link]:text-emerald-600 [&_.p-tabview-panels]:bg-white [&_.p-tabview-panels]:p-4 sm:[&_.p-tabview-panels]:p-5"
          >
              ...
              <div className="grid gap-4 xl:grid-cols-3">
                ...
                <section className={infoCardClass}>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="rounded-lg bg-slate-50 p-3"><span className="block text-xs text-slate-500">Cycle</span><strong className="mt-1 block text-slate-900">{enrollment?.cycle_name_snapshot || "—"}</strong></div>
                    <div className="rounded-lg bg-slate-50 p-3"><span className="block text-xs text-slate-500">Niveau</span><strong className="mt-1 block text-slate-900">{enrollment?.level_name_snapshot || "—"}</strong></div>
                  </div>
                </section>
              </div>
            </TabPanel>
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {guardians.map((guardian, index) => (
                  <article className={infoCardClass} key={guardian.id}>
                    <div className="flex items-center justify-between gap-2"><strong className="text-sm text-slate-900">{guardian.first_name} {guardian.last_name}</strong>{index === 0 ? <Tag value="Principal" severity="success" /> : null}</div>
                    <span className="mt-2 block text-sm text-slate-600"><i className="pi pi-phone mr-2 text-emerald-500" />{guardian.primary_phone || "Non renseigné"}</span>
                  </article>
                ))}
              </div>
            </TabPanel>
