...
export function StudentProfilePage() {
    ...
    return (
      <div className="grid min-h-[55vh] place-items-center">
        <div className="text-center">
          <span className="mx-auto grid size-11 animate-pulse place-items-center rounded-2xl bg-emerald-50 text-emerald-600">
  ...
  const className =
    enrollment &&
    "class_name_snapshot" in enrollment &&
    typeof enrollment.class_name_snapshot === "string"
      ? enrollment.class_name_snapshot
      : "Non affectée";
  const stats: Array<[string, string, string]> = [
    ["Cycle", enrollment?.cycle_name_snapshot || "—", "pi-sitemap"],
    ["Niveau", enrollment?.level_name_snapshot || "—", "pi-chart-bar"],
    ["Classe", className, "pi-users"],
  ...
  return (
    <div className="mx-auto max-w-[1480px] space-y-4 pb-8">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="pointer-events-none absolute right-8 top-7 text-[112px] text-emerald-500/[0.035]">
          ...
          <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_auto]">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-sm text-slate-500">
                  <span className="inline-flex items-center gap-2">
                    <i className="pi pi-id-card text-emerald-500" />
                    {student.matricule}
                  </span>
                  <span className="inline-flex items-center gap-2">
                    <i className="pi pi-calendar text-emerald-500" />
                    {year?.name ?? "Année non définie"}
                  </span>
                  {age !== null ? (
                    <span className="inline-flex items-center gap-2">
                      <i className="pi pi-user text-emerald-500" />
                      {age} ans
                    </span>
                  ) : null}
                </div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:w-[470px] xl:self-start">
              {stats.map(([label, value, icon]) => (
                <div
                  key={label}
                  className="flex min-h-[58px] flex-col justify-center rounded-xl border border-slate-200 bg-white/90 px-3 py-2 shadow-sm backdrop-blur-sm"
                >
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-4 sm:p-5">
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
              <section className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <dl className="divide-y divide-slate-100">
                  {identityFields.map(([label, value]) => (
                    <div
                      key={label}
                      className="grid items-center gap-2 py-3.5 sm:grid-cols-[160px_minmax(0,1fr)] sm:gap-6"
                    >
                      <dt className="text-sm font-medium text-slate-500">
                        {label}
                      </dt>
                      <dd className="min-w-0 text-sm font-semibold leading-5 text-slate-900 sm:text-left">
                        {value}
                      </dd>
                    </div>
                  ))}
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-5">
                <div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-4">
                  ...
                  <button
                    type="button"
                    className={`${resetButtonClass} grid size-9 cursor-pointer place-items-center rounded-xl border border-slate-200 text-slate-500 transition-colors hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700`}
                ...
                {primaryGuardian ? (
                  <div className="mt-4 rounded-xl bg-slate-50 p-4 ring-1 ring-inset ring-slate-100">
                    <div className="flex items-center gap-3">
                      <span className="grid size-10 place-items-center rounded-full bg-emerald-100 text-sm font-bold text-emerald-700">
                        {primaryGuardian.first_name[0]}
                        {primaryGuardian.last_name[0]}
                      </span>
                      <div>
                    ...
                    <div className="mt-4 border-t border-slate-200 pt-3 text-sm text-slate-600">
                      ...
                      {primaryGuardian.primary_phone ||
                        "Téléphone non renseigné"}
                    </div>
                  </div>
                ) : (
                  <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700">
                    Aucun responsable principal n’est renseigné.
                  </div>
                )}
              </section>
            </div>
          ...
          {activeTab === "guardians" ? (
            <div>
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <button
                  type="button"
                  className={`${resetButtonClass} inline-flex h-9 cursor-pointer items-center gap-2 rounded-lg bg-emerald-600 px-3 text-sm font-semibold text-white hover:bg-emerald-700`}
                  onClick={() => setGuardianDialog(true)}
                >
              ...
              {guardians.length ? (
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {guardians.map((guardian, index) => (
                    ...
                    >
                      ...
                      {index === 0 ? (
                        <span className="ml-2 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700">
                          Principal
                        </span>
                      ) : null}
                      <div className="mt-3 text-sm text-slate-600">
                        <i className="pi pi-phone mr-2 text-emerald-500" />
                        {guardian.primary_phone || "Non renseigné"}
                      </div>
                    </article>
                  ))}
                </div>
                ...
                <EmptyState
                  ...
                  description="Ajoutez au moins une personne à contacter."
                />
              )}
            </div>
          ) : null}
          ...
          {activeTab === "schooling" ? (
            enrollment ? (
              ...
              <EmptyState
                ...
                description="Le parcours scolaire apparaîtra ici dès qu’une inscription sera créée."
              />
            )
          ) : null}
          {activeTab === "attendance" ? (
            <EmptyState
              icon="pi-calendar"
              title="Aucune donnée d’assiduité"
              description="Les absences et retards seront regroupés ici."
            />
          ) : null}
          {activeTab === "results" ? (
            <StudentResultsPanel
          ...
          {activeTab === "documents" ? (
            enrollment ? (
              ...
              <EmptyState
                ...
                description="Une inscription active est nécessaire."
              />
            )
          ) : null}
          {activeTab === "finances" ? (
