...
export function StudentAttendancePanel({
  ...
  const unjustified = useMemo(
    () =>
      rows.filter((item) => item.justification_status !== "justified").length,
    [rows],
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-slate-950">Assiduité</h2>
          <p className="text-sm text-slate-500">
            Absences, retards et justificatifs de l’année sélectionnée.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Tag
            value={`${unjustified} à justifier`}
            severity={unjustified ? "warning" : "success"}
          />
          <Button
            label="Saisir"
            icon="pi pi-plus"
      ...
      <DataTable
        ...
        rows={10}
        emptyMessage="Aucune absence ou retard enregistré."
      >
        <Column
          field="attendance_date"
          header="Date"
          body={(row: AttendanceRow) =>
            new Date(row.attendance_date).toLocaleDateString("fr-FR")
          }
        />
        <Column
          field="slot_label"
          header="Créneau"
          body={(row: AttendanceRow) => row.slot_label || "Journée"}
        />
        <Column
          header="Type"
          body={(row: AttendanceRow) => (
      ...
      <Dialog
        header="Ajouter une absence ou un retard"
        visible={visible}
      ...
      >
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label>
            <span className="mb-1 block text-sm font-semibold">Créneau</span>
            <InputText
              className="w-full"
              value={slot}
          ...
          <label>
            <span className="mb-1 block text-sm font-semibold">Motif</span>
            <InputText
              className="w-full"
              value={reason}
