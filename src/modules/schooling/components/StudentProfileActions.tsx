...
export function StudentProfileActions({
  ...
  const finish = async (action: () => Promise<void>) => {
    ...
    try {
      ...
      setFailure("La modification n’a pas pu être enregistrée.");
    } finally {
      setBusy(false);
    }
  };
  const canConfirm =
    enrollment?.status === "draft" || enrollment?.status === "pre_registered";
    ...
    <div className="student-profile-actions">
      {canConfirm && (
        <Button
          ...
          onClick={() =>
            void finish(() =>
              changeEnrollmentStatus(enrollment.id, "confirmed"),
            )
          }
        />
      )}
      ...
      >
        <div className="schooling-form-grid">
          ...
          <Field
            ...
            onChange={(value) =>
              setStudentForm((current) => ({ ...current, birth_date: value }))
            }
          />
          <Field
            ...
            onChange={(value) =>
              setStudentForm((current) => ({ ...current, birth_place: value }))
            }
          />
          <Field
            label="Adresse"
            ...
            onChange={(value) =>
              setStudentForm((current) => ({ ...current, address: value }))
            }
          />
        </div>
      ...
      >
        <div className="schooling-form-grid">
          <Field
            ...
            onChange={(value) =>
              setGuardianForm((current) => ({ ...current, first_name: value }))
            }
          />
          <Field
            label="Nom"
            ...
            onChange={(value) =>
              setGuardianForm((current) => ({ ...current, last_name: value }))
            }
          />
          <Field
            label="Téléphone"
            value={guardianForm.primary_phone}
            onChange={(value) =>
        ...
        <Actions
          ...
          onSave={() =>
            guardian &&
            void finish(() => updateGuardian(guardian.id, guardianForm))
          }
        />
      </Dialog>
      <Dialog
        ...
        <div className="dialog-actions">
          ...
          <Button
            ...
            onClick={() =>
              enrollment &&
              void finish(() =>
                changeEnrollmentStatus(enrollment.id, "cancelled", reason),
              )
            }
          />
        </div>
