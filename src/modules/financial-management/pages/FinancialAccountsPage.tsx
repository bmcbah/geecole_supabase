...
export function FinancialAccountsPage() {
  ...
  if (!year) {
    return <Message severity="warn" text="Sélectionnez une année scolaire pour ouvrir la gestion financière." />;
  }
      ...
      <SettingsTablePanel
        ...
        dataTable={
          ...
          >
            ...
            <Column header="Reste" body={(account: FinancialAccount) => formatAmount(account.balanceAmount, account.currencyCode)} sortField="balanceAmount" sortable />
            <Column
              header="Statut"
              body={(account: FinancialAccount) => (
                <Tag value={financialAccountStatusLabels[account.status]} severity={statusSeverity[account.status]} />
              )}
              sortField="status"
              sortable
            />
      ...
      <Dialog
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button label="Générer" icon="pi pi-check" loading={generating} disabled={!enrollmentId || !paymentPlanId} onClick={() => void handleGenerate()} />
          </div>
        }
      >
        <div className="grid gap-4 pt-2">
          <label className="grid gap-2 text-sm font-medium">
            ...
            <Dropdown value={enrollmentId} options={enrollmentOptions} optionLabel="label" optionValue="value" placeholder="Sélectionner un élève" filter onChange={(event) => setEnrollmentId(event.value)} />
          </label>
          <label className="grid gap-2 text-sm font-medium">
            Plan de paiement
            <Dropdown value={paymentPlanId} options={plans} optionLabel="name" optionValue="id" placeholder="Sélectionner un plan" onChange={(event) => setPaymentPlanId(event.value)} />
          </label>
        </div>
      </Dialog>
    </div>
