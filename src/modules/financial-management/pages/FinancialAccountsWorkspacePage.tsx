...
const formatDateTime = (value: string | null) => value ? new Date(value).toLocaleString("fr-FR") : "Non généré";
...
export function FinancialAccountsWorkspacePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [result, nextEnrollments] = await Promise.all([
        listFinancialAccountsPage(institutionId, academicYearId, {
          first: viewMode === "families" || activeTab === 1 ? 0 : first,
          rows: viewMode === "families" || activeTab === 1 ? 1000 : rows,
          search,
          status,
          level,
          cycle,
          balanceState: balanceState || undefined,
          sortField: activeTab === 1 ? "studentName" : "studentName",
          sortOrder: 1,
        }),
  ...
  }, [academicYearId, activeTab, balanceState, cycle, first, institutionId, level, rows, search, status, viewMode]);
  ...
  const generationHistory = useMemo(() => [...accounts]
    .filter((account) => Boolean(account.generatedAt))
    .sort((left, right) => String(right.generatedAt).localeCompare(String(left.generatedAt))), [accounts]);
  ...
  return (
    <div className="space-y-4 pb-8">
      <PageHeader title="Dossiers financiers" description={`Rechercher, comprendre et traiter la situation financière des élèves pour ${academicYearName}.`} actions={<div className="flex gap-2"><Button label="Générer un dossier" icon="pi pi-plus" severity="secondary" outlined disabled={!enrollments.length} onClick={() => setGenerationDialogOpen(true)} /><Button label="Générer / réappliquer" icon="pi pi-refresh" onClick={() => setGlobalDialogOpen(true)} /></div>} />
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <TabView activeIndex={activeTab} onTabChange={(event) => { setActiveTab(event.index); setFirst(0); }}>
          <TabPanel header="Dossiers">
            <div className="overflow-x-auto px-1 pb-3"><div className="flex min-w-max items-end gap-2">
            ...
            {advanced ? <div className="mb-3 border-y border-emerald-100 bg-emerald-50/35 p-3"><div className="flex min-w-max items-end gap-2 overflow-x-auto">
          ...
          <TabPanel header="Historique des générations">
            <div className="mb-3 flex items-center justify-between gap-3 px-1"><div><h2 className="m-0 text-base font-semibold text-slate-950">Dossiers générés</h2><p className="mt-1 text-sm text-slate-500">Chronologie des générations actuellement conservées par dossier.</p></div><Tag value={`${generationHistory.length} génération${generationHistory.length > 1 ? "s" : ""}`} severity="info" /></div>
            <DataTable value={generationHistory} dataKey="id" paginator rows={25} rowsPerPageOptions={[25, 50, 100]} emptyMessage="Aucun dossier généré pour cette sélection." sortField="generatedAt" sortOrder={-1}>
              <Column field="generatedAt" header="Date de génération" sortable body={(row: FinancialAccount) => <strong>{formatDateTime(row.generatedAt)}</strong>} />
              <Column field="studentName" header="Élève" sortable body={(row: FinancialAccount) => <div><strong className="text-slate-950">{row.studentName}</strong><span className="mt-1 block text-xs text-slate-500">{row.matricule}</span></div>} />
              <Column field="levelName" header="Classe" sortable body={(row: FinancialAccount) => <div>{row.levelName}<span className="mt-1 block text-xs text-slate-500">{row.cycleName}</span></div>} />
              <Column header="Montant généré" body={(row: FinancialAccount) => formatAmount(row.totalAmount, row.currencyCode)} />
              <Column header="État actuel" body={(row: FinancialAccount) => <Tag value={financialAccountStatusLabels[row.status]} severity={row.balanceAmount <= 0 ? "success" : "info"} />} />
              <Column header="Action" body={(row: FinancialAccount) => <Button label="Ouvrir" icon="pi pi-arrow-right" text onClick={() => navigate(`/gestion-financiere/dossiers/${row.id}`)} />} />
            </DataTable>
          </TabPanel>
        </TabView>
      </section>
