...
const logGenerationResult = (result: FinancialGenerationResult) => {
  ...
  if (result.failed > 0) {
    console.groupCollapsed(
      `[Finance] Génération globale terminée avec ${result.failed} erreur(s)`,
    );
...
export function FinancialAccountsWorkspacePage() {
  ...
  const [generationResult, setGenerationResult] =
    useState<FinancialGenerationResult>();
  ...
  const [paymentAccount, setPaymentAccount] =
    useState<FinancialAccountDetails>();
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "[Finance] Impossible de charger les dossiers dossiers",
        cause,
      );
      setFailure("Impossible de charger les dossiers financiers.");
    } finally {
      setLoading(false);
    }
  }, [
  ...
  const pageStats = useMemo(() => {
    ...
    const balance = accounts.reduce(
      (sum, account) => sum + account.balanceAmount,
      0,
    );
    const settled = accounts.filter(
      (account) => account.balanceAmount <= 0,
    ).length;
  ...
  const levelOptions = useMemo(
    () => [
      ...
      ...Array.from(new Set(accounts.map((account) => account.levelName)))
        .filter(Boolean)
  ...
  const cycleOptions = useMemo(
    () => [
      ...
      ...Array.from(new Set(accounts.map((account) => account.cycleName)))
        .filter(Boolean)
        .sort((a, b) => a.localeCompare(b, "fr"))
        .map((value) => ({ label: value, value })),
    ],
  ...
  const activeFilterCount = [search, status, level, cycle, balanceState].filter(
    Boolean,
  ).length;
  const resetFilters = () => {
    setSearch("");
  ...
  const generateGlobal = async () => {
    ...
    try {
      ...
      notify({
        ...
        detail:
          "Réessayez ou communiquez l’identifiant de corrélation au support.",
      });
    } finally {
      setGenerating(false);
    }
  ...
  const handleGenerateUnit = async () => {
    ...
    try {
      ...
      notify({
        severity: "error",
        summary: "Génération impossible",
        detail: "Le dossier financier n’a pas pu être généré.",
      });
    } finally {
  ...
  const paymentTotal = useMemo(
    ...
    [installmentAmounts, allInstallments],
  );
  ...
  const openPayment = async (account: FinancialAccount) => {
    try {
      ...
      notify({
        ...
        detail: "Ce dossier financier ne peut pas être consulté.",
      });
    }
  };
  ...
  const toggleInstallmentCheckbox = (
    installmentId: string,
    checked: boolean,
    maxAmount: number,
  ) => {
  ...
  const settleSelectedItem = () =>
    setInstallmentAmounts(
      allInstallments.reduce<Record<string, number>>((result, installment) => {
  ...
  const handlePayment = async () => {
    ...
    const allocations = allInstallments
      .map((installment) => ({
    ...
    try {
      ...
      notify({
        ...
        detail: "L’encaissement n’a pas été enregistré.",
      });
    } finally {
  ...
  const statCards = [
    {
      ...
      value: formatAmount(pageStats.total),
      hint: "Sur la page affichée",
      icon: "pi-file",
    },
    {
      label: "Déjà encaissé",
      value: formatAmount(pageStats.paid),
      hint: "Paiements comptabilisés",
      icon: "pi-wallet",
    },
    {
      label: "Reste à payer",
      value: formatAmount(pageStats.balance),
      hint: "Solde des dossiers visibles",
      icon: "pi-chart-line",
    },
    {
      label: "Dossiers soldés",
      value: String(pageStats.settled),
      hint: `${accounts.length} dossier${accounts.length > 1 ? "s" : ""} sur cette page`,
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <PageHeader
        ...
        meta={
          <div className="flex items-center gap-2 text-sm text-slate-500">
            <MetricIcon icon="pi-wallet" />
            <strong className="font-semibold text-slate-900">
              {totalRecords}
            </strong>
            <span>dossier{totalRecords > 1 ? "s" : ""}</span>
          </div>
        }
        actions={
          <div className="flex items-center gap-2">
            <Button
              ...
              icon="pi pi-plus"
              className="h-10 rounded-xl bg-slate-100 text-slate-700 border border-slate-200 px-3 text-sm shadow-none hover:bg-slate-200"
              disabled={!enrollments.length}
              onClick={() => setGenerationDialogOpen(true)}
            />
            <Button
              label="Générer / réappliquer"
              icon="pi pi-refresh"
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        <div className="grid ps-2 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {statCards.map((card) => (
            <article
              key={card.label}
            ...
            >
              ...
              <strong className="mt-4 block text-xl font-bold tracking-tight text-slate-950">
                {card.value}
              </strong>
              ...
              <label className="block min-w-0">
                ...
                <span className="p-input-icon-left block w-full">
                  ...
                  <InputText
                    ...
                    placeholder="Élève, matricule, niveau ou cycle"
                    onChange={(event) => {
                      setFirst(0);
              ...
              <label>
                ...
                <Dropdown
                  ...
                  options={[
                    ...
                    ...Object.entries(financialAccountStatusLabels).map(
                      ([value, label]) => ({ value, label }),
                    ),
                  ]}
                  onChange={(event) => {
                    setFirst(0);
                    setStatus(String(event.value));
                  }}
                />
              </label>
              <label>
            ...
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500">
                  Affinez les dossiers sans quitter la liste.
                </p>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              ...
              <label>
                ...
                <Dropdown
                  ...
                  value={balanceState}
                  options={[
                    { label: "Tous les soldes", value: "" },
                    { label: "Soldés", value: "settled" },
                    { label: "Reste à payer", value: "outstanding" },
                  ]}
                  onChange={(event) => {
                    setFirst(0);
                    setBalanceState(event.value);
                  }}
                />
              </label>
            </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {loading ? (
          ...
          <div className="grid min-h-[360px] place-items-center p-8 text-center">
            <div className="max-w-sm">
              ...
              <p className="mt-1 text-sm leading-6 text-slate-500">
                Modifiez les filtres ou générez les frais de l’année scolaire.
              </p>
            </div>
          </div>
        ) : (
          <DataTable
            ...
            onRowClick={(event) =>
              navigate(`/gestion-financiere/dossiers/${event.data.id}`)
            }
          ...
          >
            ...
            <Column
              ...
              body={(row: FinancialAccount) => (
                <strong className="text-slate-950">
                  {formatAmount(row.totalAmount, row.currencyCode)}
                </strong>
              )}
              sortable
            ...
            <Column
              ...
              body={(row: FinancialAccount) => (
                <span className="font-semibold text-emerald-700">
                  {formatAmount(row.paidAmount, row.currencyCode)}
                </span>
              )}
              sortable
                ...
                return (
                  <div className="w-36 space-y-1">
                    ...
                    <div className="h-2 w-full overflow-hidden rounded-full bg-slate-100">
                      <div
                        className={`h-full transition-all duration-300 ${percent === 100 ? "bg-emerald-500" : percent > 50 ? "bg-blue-500" : percent > 0 ? "bg-amber-500" : "bg-slate-300"}`}
                        style={{ width: `${percent}%` }}
                      />
                    </div>
                  </div>
            ...
            <Column
              ...
              body={(row: FinancialAccount) => (
                ...
                >
                  {formatAmount(row.balanceAmount, row.currencyCode)}
                </strong>
              )}
              sortable
            ...
            <Column
              ...
              body={(row: FinancialAccount) => (
                <Tag
                  value={financialAccountStatusLabels[row.status]}
                  severity={row.balanceAmount > 0 ? "warning" : "success"}
                />
              )}
              sortable
              sortField="status"
            ...
            <Column
              header=""
              body={() => <i className="pi pi-chevron-right text-slate-400" />}
            />
          </DataTable>
        )}
      </section>
      ...
      {/* Dialogues originaux */}
      <Dialog
        header="Générer et réappliquer les frais"
        visible={confirmGlobal}
        modal
        className="w-[min(94vw,38rem)]"
        onHide={() => setConfirmGlobal(false)}
      >
        <div className="space-y-4">
          <Message
            severity="info"
            text="Les dossiers absents seront créés. Les dossiers sans paiement seront régénérés selon la grille actuelle. Les dossiers ayant déjà un encaissement seront conservés pour ne pas modifier l’historique financier."
          />
        ...
        {generationResult ? (
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ["Créés", generationResult.generated],
                ["Régénérés", generationResult.regenerated],
                ["Conservés", generationResult.skippedPaid],
                ["Erreurs", generationResult.failed],
              ].map(([label, value]) => (
                <div
                  key={String(label)}
                  className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4"
                >
                  ...
                  <div
                    className={`mt-2 text-2xl font-bold ${label === "Erreurs" ? "text-red-600" : "text-slate-950"}`}
                  >
                    {value}
                  </div>
                </div>
            ...
            {generationResult.errors.length ? (
              <>
                <Message
                  severity="error"
                  text="Certains dossiers n’ont pas pu être générés. Utilisez l’identifiant de corrélation pour le diagnostic."
                />
                <DataTable
                  value={generationResult.errors}
                  dataKey="enrollmentId"
                  paginator
                  rows={10}
                  rowsPerPageOptions={[10, 25, 50]}
                  stripedRows
                  emptyMessage="Aucune erreur retournée."
                >
                  ...
                  <Column
                    field="code"
                    header="Code"
                    body={(error: FinancialGenerationError) => (
                      <Tag value={error.code || "INCONNU"} severity="danger" />
                    )}
                  />
                  <Column
                    header="Message"
                    body={() => "Le dossier financier n’a pas pu être généré."}
                  />
                  <Column field="correlationId" header="Corrélation" />
                </DataTable>
              </>
            ) : (
              <Message
                severity="success"
                text="Tous les dossiers ont été traités sans erreur."
              />
            )}
            <div className="flex justify-end">
              <Button
                label="Fermer"
                onClick={() => setGenerationResult(undefined)}
              />
            </div>
          </div>
        ) : null}
      ...
      <Dialog
        ...
        onHide={() => setGenerationDialogOpen(false)}
      >
        <div className="space-y-4">
          <label className="block">
            <span className="mb-1.5 block text-xs font-semibold text-slate-600">
              Élève inscrit sans dossier
            </span>
            <Dropdown
              className={controlClass}
              value={enrollmentId}
              ...
              onChange={(e) => setEnrollmentId(e.value)}
            />
          </label>
          <div className="flex justify-end gap-2">
            <Button
            ...
            <Button
              label="Générer"
              icon="pi pi-check"
              loading={generatingUnit}
              disabled={!enrollmentId}
              onClick={() => void handleGenerateUnit()}
            />
          </div>
        </div>
      </Dialog>
      ...
      <Dialog
        header={`Encaissement — ${paymentAccount?.studentName ?? ""}`}
        ...
        {paymentAccount ? (
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block">
                ...
                <Dropdown
                  ...
                  onChange={(e) =>
                    initializeSelectedItem(
                      paymentAccount.items.find((i) => i.id === e.value),
                    )
                  }
                />
              </label>
              ...
              <label className="block">
                ...
                <InputText
                  className={controlClass}
                  value={reference}
                  placeholder="N° chèque, virement, reçu..."
            ...
            {selectedItem ? (
              <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-slate-900">
                    Échéances de : {selectedItem.label}
                  </h4>
                  <Button
                    label="Tout solder"
                    size="small"
                  ...
                  {allInstallments.map((installment) => {
                    ...
                    const isChecked =
                      (installmentAmounts[installment.id] ?? 0) ===
                        installment.balanceAmount && !isSolded;

                    return (
                      <div
                        key={installment.id}
                        className={`flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3 transition-colors ${isSolded ? "border-slate-200 bg-slate-100 text-slate-400 opacity-60" : "border-slate-200 bg-white"}`}
                      >
                        <div className="flex items-center gap-3">
                          {/* Case à cocher pour pré-remplir */}
                          <Checkbox
                            id={`chk-${installment.id}`}
                            checked={isSolded || isChecked}
                            disabled={isSolded}
                            onChange={(e) =>
                              toggleInstallmentCheckbox(
                                installment.id,
                                Boolean(e.checked),
                          ...
                          />
                          <label
                            htmlFor={`chk-${installment.id}`}
                            className={`block text-sm font-medium cursor-pointer ${isSolded ? "text-slate-400" : "text-slate-700"}`}
                          >
                            ...
                            <span className="block text-xs font-normal text-slate-400">
                              ...
                              {installment.dueDate
                                ? ` avant le ${formatDate(installment.dueDate)}`
                                : ""}
                            </span>
                          </label>
                        </div>
                        ...
                        <InputNumber
                          className="h-9 w-32"
                          inputClassName="h-full w-full rounded-lg border border-slate-300 text-right text-sm px-2 disabled:bg-slate-50"
                          value={
                            isSolded
                              ? installment.amount
                              : (installmentAmounts[installment.id] ?? null)
                          }
                          min={0}
                          max={installment.balanceAmount}
                          disabled={isSolded}
                          mode="currency"
                          currency={paymentAccount.currencyCode || "GNF"}
                          locale="fr-GN"
                          onChange={(e) =>
                            setInstallmentAmount(installment.id, e.value)
                          }
                        />
                      </div>
                    );
              ...
              <Message
                severity="info"
                text="Sélectionnez un frais pour répartir l’encaissement sur ses échéances."
              />
            )}
