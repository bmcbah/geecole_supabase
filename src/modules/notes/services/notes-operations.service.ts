...
export async function listOperationsPage(institutionId:string,yearId:string,mode:OperationsMode,filters:OperationsFilters):Promise<OperationsPage>{
  ...
  let query=supabase.from("notes_average_controls").select("*",{count:"exact"}).eq("institution_id",institutionId).eq("academic_year_id",yearId);if(filters.classId)query=query.eq("class_id",filters.classId);if(filters.state)query=query.eq("state",filters.state as "ready"|"incomplete"|"not_started");if(filters.search?.trim()){const term=filters.search.trim();query=query.or(`class_name.ilike.%${term}%,subject_name.ilike.%${term}%,teacher_name.ilike.%${term}%`);}query=query.order("class_name").range(filters.first,filters.first+filters.rows-1);const {data,count,error}=await query;if(error)throw error;return{rows:(data??[]).map(i=>({id:i.id,classId:i.class_id,className:i.class_name,subjectName:i.subject_name,teacherName:i.teacher_name,coefficient:i.coefficient,notesCount:i.notes_count,postponedCount:i.postponed_count,state:i.state})),total:count??0};
}
