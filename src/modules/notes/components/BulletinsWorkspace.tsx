...
export type BulletinWorkspaceMode =
  "bulletins" | "validation" | "publication" | "historique";
...
export function BulletinsWorkspace({ mode }: { mode: BulletinWorkspaceMode }) {
  ...
  async function updateBatch(next: "validated" | "published") {
    ...
    if (
      !window.confirm(
        `Confirmer le traitement de ${selected.length} bulletin(s) ?`,
      )
    )
      return;
    setProcessing(true);
    ...
    const results = await Promise.allSettled(
      selected.map((row) => changeBulletinStatus(row.id, next)),
    );
    ...
    if (selected.length - failed)
      setSuccess(`${selected.length - failed} bulletin(s) traité(s).`);
    setProcessing(false);
    await load();
  }
  ...
  return (
    <div className="space-y-4 pb-8">
      <PageHeader
        ...
        title={title}
        description={`${description} · ${year?.name ?? "Année"}`}
        actions={
          <div className="flex gap-2">
            {mode === "validation" ? (
              <Button
                label={`Valider la sélection (${selected.length})`}
                icon="pi pi-check"
                disabled={!selected.length}
                loading={processing}
                onClick={() => void updateBatch("validated")}
              />
            ) : null}
            {mode === "publication" ? (
              <Button
                label={`Publier la sélection (${selected.length})`}
                icon="pi pi-send"
                disabled={!selected.length}
                loading={processing}
                onClick={() => void updateBatch("published")}
              />
            ) : null}
            <Button
              label="Actualiser"
      ...
      <NotesDataTableToolbar
        ...
        activeCount={[query, status, classId, periodId].filter(Boolean).length}
        status={status}
        onStatus={statuses ? undefined : setStatus}
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <DataTable
          ...
          onSelectionChange={
            selectable
              ? (event) => setSelected(event.value as BulletinRow[])
              : undefined
          }
        ...
        >
          ...
          <Column
            field="status"
            header="État"
            body={(row: BulletinRow) => <Tag value={labels[row.status]} />}
          />
          <Column
            header="Actions"
            body={(row: BulletinRow) => (
              <div className="flex gap-1">
                ...
                {mode === "validation" ? (
                  <>
                    <Button
                      label="Valider"
                      text
                      onClick={() => void updateOne(row, "validated")}
                    />
                    <Button
                      label="Rejeter"
                      severity="danger"
                ...
                {mode === "publication" ? (
                  <Button
                    label="Publier"
                    text
                    onClick={() => void updateOne(row, "published")}
                  />
                ) : null}
              </div>
            )}
      ...
      >
        {preview ? (
          <pre className="max-h-[60vh] overflow-auto rounded-xl bg-slate-50 p-4 text-xs">
      ...
      >
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button
            ...
            loading={processing}
            onClick={() =>
              rejecting && void updateOne(rejecting, "rejected", reason.trim())
            }
          />
        </div>
      </Dialog>
