...
export function StudentResultsPanel(props: {
    ...
    void listStudentResults(props.institutionId, props.yearId, props.studentId)
      .then((data) => {
        ...
        setSelectedPeriod(
          (current) =>
            current ||
            data.notes[0]?.periodName ||
            data.bulletins[0]?.periodName ||
            "",
        );
      })
  ...
  const latestSnapshot = latestBulletin
    ? readSnapshot(latestBulletin.snapshot)
    : {};
    ...
    return Array.from(new Set(periodNotes.map((item) => item.subjectName)))
      ...
      .map((subjectName) => {
        ...
        return {
          ...
          notes: subjectNotes,
          officialAverage:
            typeof officialLine?.average === "number"
              ? officialLine.average
              : null,
    ...
    <div className="w-full space-y-4">
      <div className="overflow-x-auto border-b border-slate-200 bg-white">
        <div className="flex min-w-max items-center gap-6 px-1">
          {periods.map((period) => {
            const hasBulletin = bulletins.some(
              (item) => item.periodName === period,
            );
      ...
      {loading ? (
        <Message severity="info" text="Chargement des résultats…" />
      ) : null}
      ...
      {selectedPeriod ? (
          ...
          <header className="grid gap-4 border-b border-slate-200 px-4 py-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Toutes les évaluations sont regroupées par matière. La moyenne
                affichée est celle du dernier bulletin généré.
              </p>
            </div>
            {latestBulletin ? (
              <div className="flex flex-wrap items-center gap-3 lg:justify-end">
                ...
                <Tag
                  value={
                    bulletinLabels[latestBulletin.status] ??
                    latestBulletin.status
                  }
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[980px] border-collapse text-left">
              <thead className="bg-slate-50 text-xs font-semibold uppercase tracking-[0.06em] text-slate-500">
                <tr>
                  <th className="w-56 border-b border-slate-200 px-4 py-3">
                    Matière
                  </th>
                  <th className="border-b border-slate-200 px-4 py-3">
                    Évaluations
                  </th>
                  <th className="w-44 border-b border-slate-200 px-4 py-3 text-right">
                    Moyenne
                  </th>
                  <th className="w-48 border-b border-slate-200 px-4 py-3">
                    Appréciation
                  </th>
                </tr>
              </thead>
                  ...
                  <tr key={subject.subjectName} className="align-top">
                    <td className="px-4 py-4">
                      ...
                      <span className="mt-1 block text-xs text-slate-500">
                        {subject.notes.length} évaluation
                          ...
                          >
                            ...
                            <div className="sm:text-right">
                              {note.status ? (
                                <Tag
                                  value={
                                    resultLabels[note.status] ?? note.status
                                  }
                                  severity={
                      ...
                      {subject.officialAverage === null ? (
                        ...
                        <>
                          <strong className="block text-lg font-semibold text-slate-950">
                            {subject.officialAverage.toFixed(2)} /{" "}
                            {latestSnapshot.grading_scale ?? "—"}
                          </strong>
                          <span className="text-xs text-slate-500">
                            Formule du bulletin
                          </span>
        ...
        <Message
          severity="info"
          text="Aucune période ne contient encore de résultats."
        />
      ) : null}
        ...
        <section className="overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
          <header className="border-b border-slate-200 px-4 py-3">
            <h3 className="m-0 text-sm font-semibold text-slate-950">
              Versions du bulletin
            </h3>
            <p className="mt-1 text-xs text-slate-500">
              Historique immuable de la période sélectionnée.
            </p>
          </header>
          <div className="divide-y divide-slate-100">
