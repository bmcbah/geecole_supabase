  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        icon: "pi-wallet",
        to: "/parametrage/regles-financieres",
      },
      {
    ...
    () =>
      Object.fromEntries(
        navigation.map((group) => [
          group.label,
          location.pathname.startsWith(group.match),
        ]),
      ),
  );
  ...
  const sidebar = (
    <div className="flex h-full flex-col bg-slate-950 text-white">
      <div className="flex h-20 items-center justify-between border-b border-white/10 px-5">
        ...
        >
          <span className="grid size-10 place-items-center rounded-xl bg-amber-400 text-lg font-black text-slate-950 shadow-lg shadow-amber-400/20">
            G
          </span>
      ...
      <div className="border-b border-white/10 px-4 py-4">
        ...
        >
          ...
          <i className="pi pi-arrow-right text-xs text-slate-500" />
        </button>
      </div>
          ...
          return (
            <section key={group.label}>
              ...
              >
                ...
                <i
                  className={`pi pi-chevron-down text-[10px] transition-transform ${expanded ? "rotate-180" : ""}`}
                />
              </button>
      ...
      <div className="border-t border-white/10 p-4">
        <div className="flex items-center gap-3 rounded-xl bg-white/5 px-3 py-3">
          <span className="grid size-9 shrink-0 place-items-center rounded-full bg-slate-800 text-sm font-semibold text-white ring-1 ring-white/10">
      ...
      {mobileSidebarOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button
            ...
            onClick={closeMobileSidebar}
          />
          <aside className="relative h-full w-[min(88vw,288px)] shadow-2xl">
            {sidebar}
          </aside>
        </div>
      ) : null}
      ...
      <section className="min-w-0 lg:pl-72">
        <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/95 backdrop-blur">
          <div className="flex min-h-16 items-center gap-3 px-4 md:px-6 xl:px-8">
            <button
              type="button"
              className="grid size-10 shrink-0 place-items-center rounded-lg border border-slate-200 bg-white text-slate-700 shadow-sm hover:bg-slate-50 lg:hidden"
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 text-xs font-medium text-slate-500">
                ...
                <span className="truncate text-slate-700">{activePage.page}</span>
              </div>
            ...
            <button
              type="button"
              className="relative grid size-10 shrink-0 place-items-center rounded-lg border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:bg-slate-50 hover:text-slate-950"
              aria-label="Ouvrir les paramètres"
              onClick={() => void navigate("/parametrage/etablissement")}
            >
            ...
            <div className="relative">
              <button
                type="button"
                className="flex h-10 items-center gap-2 rounded-lg border border-slate-200 bg-white px-2 shadow-sm transition hover:bg-slate-50"
              ...
              >
                ...
                <span className="hidden max-w-36 truncate text-sm font-medium text-slate-700 2xl:block">
                  {user?.email}
                </span>
              ...
              {profileOpen ? (
                <div className="absolute right-0 top-12 w-64 overflow-hidden rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
                  ...
                  <button
                    type="button"
                    className="mt-2 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50"
                    onClick={() => {
                  ...
                  <button
                    type="button"
                    className="mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm font-medium text-red-600 hover:bg-red-50"
          ...
          <div className="flex items-center gap-2 overflow-x-auto border-t border-slate-100 px-4 py-2 md:hidden">
            {institutions.length > 1 ? (
              <Dropdown
                className="min-w-48"
                value={institutionId}
