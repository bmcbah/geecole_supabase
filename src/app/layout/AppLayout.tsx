...
export function AppLayout() {
  ...
  const activePage = useMemo(() => {
    for (const group of availableNavigation) {
      const item = getNavigationLinks(group).find((entry) =>
    ...
    <div className="relative flex h-full bg-white text-slate-900">
      <div className="flex w-[96px] shrink-0 flex-col bg-brand-700 text-white shadow-[4px_0_20px_rgba(15,118,110,0.2)]">
        <div className="shrink-0 px-2.5 pt-4">
          ...
          >
            <span
              className="relative block text-[28px] font-black leading-none tracking-[-0.12em] text-white"
              aria-hidden="true"
            >
              G
        ...
        <nav
          ...
          className="flex flex-1 flex-col items-stretch gap-2 px-2.5 py-3"
        >
          {availableNavigation.map((group) => {
            const active = location.pathname.startsWith(group.match);
            const selected =
              secondaryMenuOpen && selectedGroup.label === group.label;
            return (
              <button
                key={group.label}
                type="button"
              ...
              >
                ...
                <span className="w-full break-words text-[11px] font-semibold leading-tight">
                  {group.label}
                </span>
              </button>
            );
      ...
      {secondaryMenuOpen ? (
        <div className="relative flex w-[240px] min-w-0 flex-col border-r border-brand-100 bg-white lg:w-[248px] xl:w-[256px]">
          ...
          <nav
            aria-label={`Menu ${selectedGroup.label}`}
                ...
                if (item.type === "title") {
                  return (
                    <p
                      ...
                      className="px-2.5  text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400"
                    >
                      {item.label}
                    </p>
                  );
                    ...
                    {({ isActive }) => (
                      <>
                        <span className="grid pt-2 mx-1 shrink-0 place-items-center">
                          <i
                            className={`pi ${item.icon} text-center ${isActive ? "text-brand-700" : "text-slate-400 group-hover:text-brand-700"}`}
                          />
                        </span>
  ...
  return (
    <div className="min-h-screen bg-surface-page text-text-primary">
      <aside
        className={`fixed inset-y-0 left-0 z-40 hidden lg:block ${sidebarWidth}`}
      >
        {sidebar}
      </aside>
      ...
      {mobileSidebarOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside
            className={`relative h-full shadow-2xl ${secondaryMenuOpen ? "w-[min(96vw,336px)]" : "w-[96px]"}`}
          >
            {sidebar}
          </aside>
        </div>
      ...
      <section
        className={`min-w-0 transition-[padding] duration-200 ${contentPadding}`}
      >
        ...
        <main className="mx-auto max-w-[1440px] p-4 md:p-2 xl:p-4">
          <Outlet />
        </main>
      </section>
    </div>
